...
def _run_sample_review(state: RunState) -> None:
    ...
    required = ["survey.md", "evidence_pack.json", "check_report.json"]
    missing = [name for name in required if not (sample_dir / name).exists()]
    if missing:
        state.fail(
            "No local sample artifacts were found. Run the CLI once or start a live review.",
    ...
    for name in required:
        shutil.copy2(sample_dir / name, state.output_dir / name)
    trace = sample_dir / "skill_trace.json"
    if trace.exists():
        shutil.copy2(trace, state.output_dir / "skill_trace.json")
    _finalize_artifacts(state)
...
def _sample_events(state: RunState) -> list[dict[str, Any]]:
    return [
        ...
        {"type": "components_ready", "tools": ["build_literature_kb", "list_evidence", "read_evidence", "search_literature", "read_context"]},
        {"type": "run_started", "task_preview": f"Sample replay for {state.topic}", "available_tools": ["build_literature_kb", "list_evidence", "read_evidence"]},
        {"type": "iteration_started", "iteration": 1, "max_iterations": 15},
        {"type": "context_prepared", "iteration": 1, "message_count": 2, "tool_count": 9},
        ...
        {"type": "iteration_started", "iteration": 2, "max_iterations": 15},
        {"type": "llm_response", "iteration": 2, "mode": "final_content", "content_chars": 21472},
        {"type": "stop_condition", "iteration": 2, "name": "CitationVerifier", "passed": True, "report": {"status": "pass"}},
